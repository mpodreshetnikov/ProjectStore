...
- **21 commands** — `bind`, `scaffold`, `status`, `adr`, `spec`, `epic`, `story` (with `plan`/`close` lifecycle gates), `kanban`, `research`, `concept`, `meeting`, `runbook`, `diagram`, `search`, `review`, `statusline`, `doctor`, `reconcile`, `codemap`, `graph`, `agents`
