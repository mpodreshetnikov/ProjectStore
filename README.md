...
├── ops/               ← runbooks
├── diagrams/       ← mermaid figures, linked from the artifact that reasons
...
/projectstore:migrate       # bring an older vault up to date with the plugin
...
- **22 commands** — `bind`, `scaffold`, `status`, `adr`, `spec`, `epic`, `story` (with `plan`/`close` lifecycle gates), `kanban`, `research`, `concept`, `meeting`, `runbook`, `diagram`, `search`, `review`, `statusline`, `doctor`, `reconcile`, `migrate`, `codemap`, `graph`, `agents`
...
- **Folder meaning is data (v0.25)** — each layout folder declares a `purpose` and a `not_this` rule, localized in the layout's own strings sidecar. Two vaults on one layout get byte-identical folder READMEs, and the four boundaries that were never written down — research↔concept, research↔adr, meeting↔adr, runbook↔spec — now are, in the folder README and in the creating command's prompt.
- **Existing vaults are brought forward, not left behind (v0.25)** — `/projectstore:migrate` is a registry of idempotent, self-detecting transforms, not a one-off script. Whether a vault needs one is derived from its own state, so the same command serves a vault three releases behind and a vault already current, and future releases add an entry instead of a sentence in a changelog. It previews every byte it replaces, archives the previous contents under `<vault>/.projectstore/migrations/`, and never touches a preamble whose owner marked it `mine`.
...
|---|---|---|
| **v0.25** | Folder meaning stops being improvised. Until now the only place a folder's purpose existed was a sentence the model composed at scaffold time (`commands/scaffold.md` asked it to substitute `{{folder_description}}` "based on the folder kind") — after which `folderPurpose()` canonized that improvisation and SessionStart injected it every session as the folder's stated purpose. Two vaults on one layout therefore disagreed about what `research/` is for, and re-scaffolding produced different text again. Now every `folders[]` entry declares a `purpose` and a `not_this` id resolved through `scaffold/layouts/<layout>.strings.json`, composed in code (`renderFolderReadme`) rather than in prose, so the READMEs are byte-identical across vaults. The boundary rule lives in its own `## Not this` section, deliberately BELOW the preamble: `folderPurpose()` cuts at the first `## `, so the rule never competes with the purpose for the skeleton's 160-character cell. Four boundaries that had never been written down — research↔concept, research↔adr, meeting↔adr, runbook↔spec — are now stated in both the folder README (localized) and the creating command's prompt. `diagrams/` stops being a folder nothing could fill: it gets `/projectstore:diagram`, a template in all six languages and a review checklist. Doctor gains `folder-purpose`, which lints **only** READMEs carrying the `<!-- projectstore:purpose -->` marker this code writes — deleting that line is the documented way to keep your own wording, and it is why an existing vault does not start warning on every folder. **Migration**: run `/projectstore:migrate` once — it previews every folder README it would rewrite, keeps the index table and everything below it byte-for-byte, copies the previous contents to `<vault>/.projectstore/migrations/` first, and skips any README whose table it does not recognize rather than guessing. Doctor tells you when a vault has pending migrations. A folder whose wording you want to keep: change `managed` to `mine` on its marker line — never delete the line, which reads as a vault nobody has migrated yet. Custom-locale users get one install issue until they add `diagram.md.tmpl` for their language |  ✅ current |
